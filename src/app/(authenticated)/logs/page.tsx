    ...
    <div className="h-full bg-panel flex flex-col">
      <div className="sticky top-0 z-10 bg-panel border-b border-line px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <ListBullets size={20} weight="bold" className="text-muted" />
          <h1 className="text-lg font-bold text-ink">Logs de Mensagens (n8n)</h1>
        </div>
        <button
          onClick={fetchLogs}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-muted gap-3">
            ...
            <p className="text-xs text-muted max-w-md text-center">
              ...
              <code className="mx-1 px-1.5 py-0.5 bg-panel-2 rounded text-muted">/api/webhooks/n8n-outbound</code>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-line">
              {logs.map((log) => (
                <tr key={log.id} className="hover:bg-void">
                  ...
                  <td className="px-6 py-3 text-muted capitalize">{log.source}</td>
                  <td className="px-6 py-3 text-muted">{log.phone || '—'}</td>
                  <td className="px-6 py-3 text-muted max-w-md truncate">{log.content || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
