...
__export(schema_exports, {
  ...
  expenses: () => expenses,
  featureScreenshots: () => featureScreenshots,
  funnelBlockTypeEnum: () => funnelBlockTypeEnum,
  funnelBlocks: () => funnelBlocks,
  funnelBranchEnum: () => funnelBranchEnum,
  funnelClickEvents: () => funnelClickEvents,
  funnelConnections: () => funnelConnections,
  funnelExecutionStatusEnum: () => funnelExecutionStatusEnum,
  funnelExecutions: () => funnelExecutions,
  funnelResponseEvents: () => funnelResponseEvents,
  funnelTriggerEnum: () => funnelTriggerEnum,
  integrationMessageLogs: () => integrationMessageLogs,
  integrationSecrets: () => integrationSecrets,
  integrationStatusEnum: () => integrationStatusEnum,
  integrations: () => integrations,
  leadActivities: () => leadActivities,
  leadActivityTypeEnum: () => leadActivityTypeEnum,
  leadSourceSubmissions: () => leadSourceSubmissions,
  leadStageHistory: () => leadStageHistory,
  leadTags: () => leadTags,
  leads: () => leads,
  memberStatusEnum: () => memberStatusEnum,
  messageFunnels: () => messageFunnels,
  ...
  quickReplyScopeEnum: () => quickReplyScopeEnum,
  quickReplySteps: () => quickReplySteps,
  sessions: () => sessions,
...
var memberStatusEnum = pgEnum("member_status", ["active", "invited", "disabled"]);
var integrationStatusEnum = pgEnum("integration_status", ["active", "disabled"]);
var leadActivityTypeEnum = pgEnum("lead_activity_type", ["note", "call", "whatsapp", "email", "system"]);
var customFieldTypeEnum = pgEnum("custom_field_type", ["text", "number", "date", "bool", "select", "multi_select", "json"]);
var tiers = pgTable("tiers", {
...
var pipelineStages = pgTable("pipeline_stages", {
  id: uuid("id").defaultRandom().primaryKey(),
...
var integrationSecrets = pgTable("integration_secrets", {
  id: uuid("id").defaultRandom().primaryKey(),
...
var leads = pgTable("leads", {
  ...
  orgPhoneUnique: uniqueIndex("leads_org_phone_unique").on(t.organizationId, t.phone).where(sql`deleted_at IS NULL AND phone IS NOT NULL AND phone != ''`),
  integrationExternalIdUnique: uniqueIndex("leads_integration_external_id_unique").on(t.integrationId, t.externalId).where(sql`deleted_at IS NULL AND external_id IS NOT NULL`)
}));
...
var leadActivities = pgTable("lead_activities", {
  ...
  evolutionMsgIdUnique: uniqueIndex("lead_activities_evolution_msgid_unique").on(sql`(metadata->>'evolution_message_id')`).where(sql`metadata->>'evolution_message_id' IS NOT NULL`),
  whatsappMsgIdUnique: uniqueIndex("lead_activities_whatsapp_msgid_unique").on(sql`(metadata->>'whatsapp_message_id')`).where(sql`metadata->>'whatsapp_message_id' IS NOT NULL`),
  instagramMsgIdUnique: uniqueIndex("lead_activities_instagram_msgid_unique").on(sql`(metadata->>'instagram_message_id')`).where(sql`metadata->>'instagram_message_id' IS NOT NULL`),
  zapiMsgIdUnique: uniqueIndex("lead_activities_zapi_msgid_unique").on(sql`(metadata->>'zapi_message_id')`).where(sql`metadata->>'zapi_message_id' IS NOT NULL`)
}));
var pinnedMessages = pgTable("pinned_messages", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").notNull().references(() => organizations.id),
  leadId: uuid("lead_id").notNull().references(() => leads.id, { onDelete: "cascade" }),
...
var customFieldCategories = pgTable("custom_field_categories", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").notNull().references(() => organizations.id),
  name: text("name").notNull(),
  rank: numeric("rank", { precision: 20, scale: 6 }).notNull(),
...
var customFieldDefinitions = pgTable("custom_field_definitions", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").notNull().references(() => organizations.id),
...
var leadSourceSubmissions = pgTable("lead_source_submissions", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").notNull().references(() => organizations.id),
  ...
  dedupeUnique: uniqueIndex("lead_source_submissions_dedupe_unique").on(t.organizationId, t.source, t.externalId).where(sql`external_id IS NOT NULL`)
...
var integrationMessageLogs = pgTable("integration_message_logs", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").notNull().references(() => organizations.id),
  source: text("source").notNull(),
...
var setupTokens = pgTable("setup_tokens", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").notNull().references(() => organizations.id),
  token: text("token").notNull().unique(),
...
var funnelTriggerEnum = pgEnum("funnel_trigger", ["novo_pago", "novo_recuperacao", "geracaowhatsapp", "pedido_figurinha", "abandono_preco", "lead_site_evento", "lead_agenda_ascensao"]);
var funnelBlockTypeEnum = pgEnum("funnel_block_type", ["trigger", "message", "wait", "condition", "end", "move_stage"]);
var funnelBranchEnum = pgEnum("funnel_branch", ["default", "yes", "no"]);
var funnelExecutionStatusEnum = pgEnum("funnel_execution_status", ["running", "waiting", "waiting_condition", "completed", "stopped"]);
var messageFunnels = pgTable("message_funnels", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").notNull().references(() => organizations.id),
  name: text("name").notNull(),
...
var funnelBlocks = pgTable("funnel_blocks", {
  id: uuid("id").defaultRandom().primaryKey(),
...
var funnelConnections = pgTable("funnel_connections", {
  id: uuid("id").defaultRandom().primaryKey(),
  funnelId: uuid("funnel_id").notNull().references(() => messageFunnels.id, { onDelete: "cascade" }),
...
var funnelExecutions = pgTable("funnel_executions", {
  id: uuid("id").defaultRandom().primaryKey(),
  funnelId: uuid("funnel_id").notNull().references(() => messageFunnels.id, { onDelete: "cascade" }),
  organizationId: uuid("organization_id").notNull(),
  leadId: uuid("lead_id").notNull().references(() => leads.id, { onDelete: "cascade" }),
...
var funnelClickEvents = pgTable("funnel_click_events", {
  id: uuid("id").defaultRandom().primaryKey(),
...
var funnelResponseEvents = pgTable("funnel_response_events", {
  id: uuid("id").defaultRandom().primaryKey(),
  executionId: uuid("execution_id").notNull().references(() => funnelExecutions.id, { onDelete: "cascade" }),
  blockId: uuid("block_id").notNull(),
...
var quickReplyScopeEnum = pgEnum("quick_reply_scope", ["shared", "personal"]);
var quickReplies = pgTable("quick_replies", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").notNull().references(() => organizations.id),
  ...
  sharedShortcutUnique: uniqueIndex("quick_replies_shared_shortcut_unique").on(t.organizationId, sql`lower(${t.shortcut})`).where(sql`${t.scope} = 'shared' and ${t.deletedAt} is null`),
  personalShortcutUnique: uniqueIndex("quick_replies_personal_shortcut_unique").on(t.organizationId, t.createdByMemberId, sql`lower(${t.shortcut})`).where(sql`${t.scope} = 'personal' and ${t.deletedAt} is null`)
}));
var quickReplySteps = pgTable("quick_reply_steps", {
  id: uuid("id").defaultRandom().primaryKey(),
...
var expenses = pgTable("expenses", {
  ...
  parentDueDateUnique: uniqueIndex("expenses_parent_due_date_unique").on(t.parentExpenseId, t.dueDate).where(sql`${t.parentExpenseId} is not null and ${t.deletedAt} is null`)
}));
var featureScreenshots = pgTable("feature_screenshots", {
  id: uuid("id").defaultRandom().primaryKey(),
...
function getDb() {
  if (!_db) {
    ...
    if (!url2)
      throw new Error("DATABASE_URL n\xE3o definida nas vari\xE1veis de ambiente");
    const pool = new Pool2({ connectionString: url2 });
...
// src/lib/zapi.ts
var ZAPI_INTEGRATION_TYPE = "whatsapp_zapi";
var ZAPI_INTEGRATION_NAME = "WhatsApp Z-API";
var BASE = "https://api.z-api.io/instances";
async function getZapiCredentials(organizationId) {
  const [integration] = await db.select({ id: integrations.id, config: integrations.config }).from(integrations).where(and(
    eq(integrations.organizationId, organizationId),
    eq(integrations.type, ZAPI_INTEGRATION_TYPE),
    isNull(integrations.deletedAt)
  )).limit(1);
  if (!integration)
    throw new Error("Integra\xE7\xE3o Z-API n\xE3o configurada.");
  const [secretRow] = await db.select({ secret: integrationSecrets.secret }).from(integrationSecrets).where(eq(integrationSecrets.integrationId, integration.id)).limit(1);
  const config = integration.config ?? {};
  const secret = secretRow?.secret ?? {};
  if (!config.instance_id || !secret.instance_token) {
    throw new Error("Credenciais da Z-API incompletas: falta o ID ou o token da inst\xE2ncia.");
  }
  return {
...
async function temZapiConfigurado(organizationId) {
  try {
...
function url(creds, caminho) {
  return `${BASE}/${creds.instanceId}/token/${creds.instanceToken}/${caminho}`;
}
function erroZapi(data, texto, status, fallback) {
  if (typeof data?.error === "string" && data.error)
    return data.error;
  if (typeof data?.message === "string" && data.message)
    return data.message;
  if (texto && texto.length < 200 && !texto.trim().startsWith("<"))
    return texto;
  return `${fallback} (HTTP ${status})`;
}
async function chamar(creds, caminho, init = {}, fallbackErro = "Falha na chamada \xE0 Z-API") {
  ...
  if (!res.ok)
    throw new Error(erroZapi(data, texto, res.status, fallbackErro));
  return data;
}
function destinatario(phone) {
  return phone.replace(/\D/g, "");
}
async function comCorrecaoDeNumero(phone, enviar) {
  ...
  for (const numero of tentativas) {
    try {
      ...
      if (primeiroErro === null)
        primeiroErro = err;
      console.warn("[zapi] envio falhou para " + numero + "; tentando a outra forma do n\xFAmero", err);
    }
  }
  throw primeiroErro ?? new Error("Falha ao enviar pela Z-API");
}
async function sendZapiMessage(organizationId, phone, text2) {
  const creds = await getZapiCredentials(organizationId);
  return comCorrecaoDeNumero(
    phone,
    (numero) => chamar(
      creds,
      "send-text",
      { body: { phone: destinatario(numero), message: text2 } },
      "Falha ao enviar mensagem pela Z-API"
    )
  );
}
async function sendZapiMedia(organizationId, phone, mediaType, mediaUrl, caption, fileName) {
  const creds = await getZapiCredentials(organizationId);
  return comCorrecaoDeNumero(phone, (numero) => {
    ...
    if (mediaType === "video") {
      ...
      }, "Falha ao enviar v\xEDdeo pela Z-API");
    }
    if (mediaType === "audio") {
      return chamar(creds, "send-audio", {
        body: { phone: to, audio: mediaUrl }
      }, "Falha ao enviar \xE1udio pela Z-API");
    }
...
function extensaoDe(fileName, mediaUrl) {
  const doNome = fileName?.split(".").pop();
  if (doNome && /^[a-z0-9]{1,5}$/i.test(doNome))
    return doNome.toLowerCase();
  const semQuery = (mediaUrl || "").split("?")[0];
  const daUrl = semQuery.split(".").pop();
  if (daUrl && /^[a-z0-9]{1,5}$/i.test(daUrl))
    return daUrl.toLowerCase();
  return "bin";
}
async function deleteZapiMessage(organizationId, phone, messageId, owner = true) {
  const creds = await getZapiCredentials(organizationId);
...
async function fetchZapiStatus(organizationId) {
  const creds = await getZapiCredentials(organizationId);
...
async function fetchZapiChats(organizationId, page = 1, pageSize = 50) {
  const creds = await getZapiCredentials(organizationId);
...
async function rehospedarMidiaZapi(organizationId, mediaUrl, messageId) {
  try {
    ...
    console.error("[zapi] falha ao re-hospedar m\xEDdia", err);
    return null;
  }
}
async function rehospedarFotoPerfil(organizationId, fotoUrl, phone) {
  try {
